...
async function isTextAIGenerated(text) {
    const anthropic = initializeAnthropic();
  
    const prompt = `You are a text analysis assistant. Analyze the following text and determine if it is likely to have been generated by an AI language model or written by a human:
  
  ${text}
  
  Provide a response indicating your assessment of whether the text is "ai-generated" or "Human-written".`;
    ...
    return new Promise((resolve, reject) => {
      stream.on('end', () => {
        const reply = output.trim().toLowerCase();
        resolve(reply);
      });
      stream.on('error', (error) => {
        reject(error);
